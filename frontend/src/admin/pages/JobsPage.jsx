...
export const JobsPage = () => {
  ...
  const jobColumns = [
    ...
    },
    {
      ...
      field: 'department',
      render: (row) => (
        <Badge variant={row.department === 'AI & Innovation' ? 'primary' : 'slate'}>
          {row.department}
        </Badge>
      )
    },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            >
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                <Badge variant="primary">{selectedJob.department}</Badge>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                {selectedJob.responsibilities?.map((resp, idx) => (
                  <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.84375rem', color: 'var(--text-primary)' }}>
                    <CheckCircle2 size={15} style={{ color: 'var(--primary)', marginTop: '2px' }} />
                    <span>{resp}</span>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                {selectedJob.requirements?.map((req, idx) => (
                  <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.84375rem', color: 'var(--text-primary)' }}>
                    <CheckCircle2 size={15} style={{ color: '#059669', marginTop: '2px' }} />
                    <span>{req}</span>
                  </div>
                ))}
              </div>
