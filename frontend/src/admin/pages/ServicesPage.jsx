    ...
    },
    {
      ...
      render: (row) => (
        <span style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
          {row.process ? `${row.process.length} Agile Steps` : '4 Steps'}
        </span>
      )
    },
    {
      ...
      field: 'isActive',
      render: (row) => (
        <Badge variant={row.isActive !== false ? 'emerald' : 'rose'}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: 'var(--radius-sm)', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFFFFF' }}>
                  {getServiceIcon(selectedService.icon)}
                </div>
                <div>
                  <h4 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'var(--text-primary)' }}>{selectedService.title}</h4>
