    ...
    <div className="page-container animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '1.25rem' }}>
        ...
        <div className="glass-card" style={{ padding: '1.25rem', height: 'fit-content' }}>
          <h3 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: '0.75rem', color: '#ffffff' }}>
            ...
            {PRESET_ENDPOINTS.map((preset, idx) => (
              <button
                ...
                style={{
                  ...
                  fontSize: '0.8125rem',
                  backgroundColor: endpoint === preset.endpoint && method === preset.method ? 'var(--bg-tertiary)' : 'transparent',
                  border: endpoint === preset.endpoint && method === preset.method ? '1px solid var(--border-medium)' : '1px solid transparent'
                }}
              >
                <Badge
                  variant={preset.method === 'GET' ? 'white' : 'slate'}
                  size="sm"
          ...
          <div className="glass-card" style={{ padding: '1.25rem' }}>
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
              <select
                ...
                style={{ width: '100px', fontWeight: 700, color: '#ffffff' }}
                value={method}
          ...
          <div className="glass-card" style={{ padding: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <h3 style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#ffffff' }}>Live Response Payload</h3>
                {response && (
                  <>
            ...
            {response ? (
              <JsonViewer data={response.data || response.error || response} maxHeight="420px" />
            ) : (
